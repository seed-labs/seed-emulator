    ...
    def test_seedops_capabilities_shape(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                self.assertIn("routing_protocol_summary", payload["supported_actions"])
                self.assertIn("routing_looking_glass", payload["supported_actions"])
                self.assertIn("seedops_capabilities", payload["tool_names"])
                self.assertIn("workspace_set_visibility", payload["tool_names"])
                self.assertIn("workspace_get_visibility", payload["tool_names"])
