...
class Simulation():
    ...
    def __init__(self, node_total=10, override=False) -> None:
        self.node_total = node_total
        self.loss_topology = [[0 for col in range(self.node_total)] for row in range(self.node_total)]
        self.delay_topology = [[0.0 for col in range(self.node_total)] for row in range(self.node_total)]
    ...
    def __init_node_info(self):
        ...
        #         node_info = NodeInfo(id, container_id="", ipaddress="",
        ...
        for id in range(self.node_total):
            node_info = NodeInfo(id, "", "")   
            self.node_info["node_info"].append(node_info)
        ...
        for i in range(self.node_total):
            ...
            for j in range(i+1, self.node_total):
    ...
    def __update_loss_and_delay(self):
        ...
        conditionModel = None
        if lossModel.getChannelType() == "ThreeGpp":
            conditionModel = lossModel.getChannelConditionModel()
        elif lossModel.getChannelType() == "ThreeGppV2v":
            conditionModel = lossModel.getChannelConditionModel()
            # conditionModel = lossModel.getChannelConditionModel().buildingsChannelConditionModel
        ...
        for i in range(self.node_total):
            ...
            for j in range(i+1, self.node_total):
                ...
                if conditionModel == None:
                    self.csvWrite([self.iteration, i, j, x1, y1, x2, y2, dist, "NA", loss_rate, delay])
                else:
                    self.csvWrite([self.iteration, i, j, x1, y1, x2, y2, dist, conditionModel.getChannelCondition(node_a, node_b).getLosCondition().value, loss_rate, delay])
    ...
    def csvInit(self):
        directory_path = '/tmp/seedsim/siminfo'
        ...
        with open('/tmp/seedsim/siminfo/siminfo.csv', 'w', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=' ')
            columns = "Iter tx_node_id rx_node_id TxPosX[m] TxPosY[m] RxPosX[m] RxPosY[m] Distance[m] ChannelState LossRate Delay"
    ...
    def compile_tc_commands(self, iteration, iter_duration=1, sleep_duration=0.1, log_enabled=False):
        '''
        @brief compile tc commands per nodes and iterations from the csv file created from Simulation::move_nodes method. 
        ...
        is_log_enabled = "true" if log_enabled else "false"

        if not directory.exists():
            directory.mkdir(parents=True)

        with open('/tmp/seedsim/setting/info.txt', 'w') as file:
            file.write("{}-{}-start_time-{}".format(iteration, iter_duration, sleep_duration))

        with open('/tmp/seedsim/tc_runner.sh', 'w') as file:
            file.write(TC_RUNNER_SCRIPT.format(isLogEnabled=is_log_enabled,
                                            ping_time=12))
        ...
        for iter in range(iteration):
            for node_id in range(self.node_total):
    ...
    def compile_position_infos(self):
        '''
        @brief compile node position infos into json file format. 
        The files will be located in /tmp/node_info.
        Before calling this method, it is essential to check whether the Emulator is running.
    ...
    def update_loss_and_delay_on_containers(self):
        ...
        @return self, for chaining api calls.
        '''
        for k in range(self.node_total):   
