...
for i in range(total):
    ...
    node_id.addSharedFolder('/setting', '/tmp/seedsim/setting')
    node_id.addSharedFolder('/tc_command', '/tmp/seedsim/tc_command/{node_id}'.format(node_id=i))
    node[i].appendStartCommand('chmod +x /tc_command/*')
    
    node_id.importFile(hostpath='/tmp/seedsim/tc_runner.sh', containerpath='/tc_runner.sh')
    node[i].appendStartCommand('chmod +x /tc_runner.sh')
...
emu.compile(Docker(internetMapEnabled=False), './output_{}'.format(NODE_TOTAL), override = True)
# os.system("rm -rf /tmp/node_info/*")
# os.mkdir("/tmp/node_info")

shutil.copy('./z_start.sh', './output_{}'.format(NODE_TOTAL))