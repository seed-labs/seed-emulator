...
if [ ! -e /tor-config-done ]; then
	...
	if [  -z "${{TOR_HS_PORT_1}}" ]; then
	  TOR_HS_PORT=80
	fi
	if [ -z "${{TOR_HS_ADDR}}" ]; then
	  TOR_HS_ADDR=127.0.0.1
	fi
	echo -e "HiddenServicePort ${{TOR_HS_PORT_1}} ${{TOR_HS_ADDR}}:${{TOR_HS_PORT_1}}" >> /etc/tor/torrc
    if [ ! -z "${{TOR_HS_PORT_2}}" ]; then
	  echo -e "HiddenServicePort ${{TOR_HS_PORT_2}} ${{TOR_HS_ADDR}}:${{TOR_HS_PORT_2}}" >> /etc/tor/torrc
	fi
    if [ ! -z "${{TOR_HS_PORT_3}}" ]; then
	  echo -e "HiddenServicePort ${{TOR_HS_PORT_3}} ${{TOR_HS_ADDR}}:${{TOR_HS_PORT_3}}" >> /etc/tor/torrc
	fi
    if [ ! -z "${{TOR_HS_PORT_4}}" ]; then
	  echo -e "HiddenServicePort ${{TOR_HS_PORT_4}} ${{TOR_HS_ADDR}}:${{TOR_HS_PORT_4}}" >> /etc/tor/torrc
	fi
	;;
      *)
...
class TorServer(Server):
    ...
    def setLink(self, addr: str, port: list(int)) -> TorServer:
        """!
    ...
    def linkByVnode(self, vname: str, port: list(int)) -> TorServer:
        """!
        ...
        assert len(port) <= 4, "linkByVnode(): TorServer can have at most 3 ports. "
        ...
        if self.getRole() == "HS" and len(self.getLink()) != 0:
            ...
            if type(port) is list :
                for i in range(len(port)):
                    node.appendStartCommand("export TOR_HS_PORT_{}={}".format(i+1, port[i]))
            else:
                node.appendStartCommand("export TOR_HS_PORT_1={}".format(port))
