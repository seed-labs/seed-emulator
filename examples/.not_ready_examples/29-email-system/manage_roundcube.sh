...
start_roundcube() {
    ...
    if [ $? -eq 0 ]; then
        ...
        echo ""
        echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        echo "  📬 Roundcube Webmail: http://localhost:8081"
        echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        echo ""
        echo "测试账户："
        echo "  • alice@seedemail.net (密码: password123)"
        echo "  • bob@seedemail.net (密码: password123)"
        echo ""
        echo "支持的邮件域："
        echo "  • seedemail.net (Public Email)"
        echo "  • corporate.local (Enterprise)"  
        echo "  • smallbiz.org (Small Business)"
        echo ""
    else
...
logs_roundcube() {
    ...
    docker logs -f roundcube-webmail
}
...
create_test_accounts() {
    ...
    # alice@seedemail.net
    printf "password123\npassword123\n" | docker exec -i mail-150-seedemail setup email add alice@seedemail.net 2>/dev/null
    if [ $? -eq 0 ]; then
        print_success "创建 alice@seedemail.net"
    else
        print_warning "alice@seedemail.net 可能已存在"
    fi
    
    # bob@seedemail.net
    printf "password123\npassword123\n" | docker exec -i mail-150-seedemail setup email add bob@seedemail.net 2>/dev/null
    if [ $? -eq 0 ]; then
        print_success "创建 bob@seedemail.net"
    else
        print_warning "bob@seedemail.net 可能已存在"
    fi
    
    # admin@corporate.local
    printf "password123\npassword123\n" | docker exec -i mail-151-corporate setup email add admin@corporate.local 2>/dev/null
    if [ $? -eq 0 ]; then
        print_success "创建 admin@corporate.local"
    else
        print_warning "admin@corporate.local 可能已存在"
    fi
    
    # info@smallbiz.org
    printf "password123\npassword123\n" | docker exec -i mail-152-smallbiz setup email add info@smallbiz.org 2>/dev/null
    if [ $? -eq 0 ]; then
        print_success "创建 info@smallbiz.org"
    else
        print_warning "info@smallbiz.org 可能已存在"
    fi
