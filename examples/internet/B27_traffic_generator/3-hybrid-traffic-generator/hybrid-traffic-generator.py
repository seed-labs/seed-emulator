...
def run(dumpfile=None):
    ...
    traffic_service.install(
            'hybrid-generator', 
            TrafficServiceType.HYBRID_GENERATOR
    ).addReceivers(hosts=["hybrid-receiver"])
    ...
    emu.addBinding(Binding('hybrid-generator', 
                   filter = Filter(asn = 150, nodeName='hybrid-generator')))
    emu.addBinding(Binding('hybrid-receiver', 
                   filter = Filter(asn = 162, nodeName='hybrid-receiver')))
