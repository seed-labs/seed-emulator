...
sudo ovs-docker add-port br1 eth0 host-b1 --ipaddress=10.9.2.1/24 --gateway=10.9.2.254
sudo ovs-docker add-port br1 eth0 host-b2 --ipaddress=10.9.2.2/24 --gateway=10.9.2.254
sudo ovs-docker add-port br1 eth0 host-c1 --ipaddress=10.9.3.1/24 --gateway=10.9.3.254
sudo ovs-docker add-port br1 eth0 host-c2 --ipaddress=10.9.3.2/24 --gateway=10.9.3.254

sudo ip link add veth-faucet type veth peer name veth-faucet-ovs
sudo ovs-vsctl add-port br1 veth-faucet-ovs
sudo ip addr add 10.9.3.200/24 dev veth-faucet
sudo ip link set veth-faucet up
sudo ip link set veth-faucet-ovs up

sudo ovs-docker add-port br1 sdn0 bgp-router --ipaddress=10.9.3.100/24 --gateway=10.9.3.254
