...
# The following information is emulator specific and needs to change accordingly
###############################################################
...
class test_user_contract:
    ...
    def fund_test_account(self):
        logging.info("Sending a fund request to the Faucet server")
        data = {'address': self.test_address, 'amount': 1}
        response = requests.post(self.faucet_url, 
    ...
    def check_eth_price(self):
        user_contract = self.web3.eth.contract(address=self.user_contract_address, 
        ...
        request_eth_price_data_function = user_contract.functions.\
                                             requestETHPriceData(api, path)
        tx = {
        ...
        signed_tx = self.web3.eth.account.signTransaction(request_eth_price_data_tx, 
                                                          self.private_key)
        tx_hash = self.web3.eth.sendRawTransaction(signed_tx.rawTransaction)
        logging.info(f"Sent transaction for ETH price data request. Hash: {tx_hash.hex()}")

        request_eth_price_data_receipt = self.web3.eth.wait_for_transaction_receipt(tx_hash,
                                                 timeout=300)
        logging.info(f"Status of Transaction receipt for ETH price data request: " +
                     f"{request_eth_price_data_receipt.status}")
