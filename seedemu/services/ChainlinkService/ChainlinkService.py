...
class ChainlinkServer(Server):
    ...
    def install(self, node: Node):
        ...
        node.setFile('/send_fund_request.sh', ChainlinkFileTemplate['send_get_eth_request'].format(faucet_server_url=self.__faucet_server_url, faucet_server_port=self.__faucet_server_port, rpc_url=self.__eth_server_url, rpc_port=self.__eth_server_http_port))
        node.appendStartCommand('bash /send_fund_request.sh')
        node.setFile('/check_init_node.sh', ChainlinkFileTemplate['check_init_node'].format(init_node_url=self.__init_server_url,
                                                                                                   init_node_port=self.__init_server_port,
                                                                                                   contract_name =LinkTokenDeploymentTemplate['link_token_name']))
        node.appendStartCommand('bash /check_init_node.sh')
        self.__deploy_oracle_contract(node)
        node.setFile('/send_flask_request.sh', ChainlinkFileTemplate['send_flask_request'].format(init_node_url=self.__init_server_url, 
                                                                                                         init_node_port=self.__init_server_port,
                                                                                                         contract_name=self.__name))
        node.appendStartCommand('bash /send_flask_request.sh')
        node.setFile('/create_chainlink_jobs.sh', ChainlinkFileTemplate['create_jobs'])
        node.appendStartCommand('bash /create_chainlink_jobs.sh')
        node.appendStartCommand('tail -f chainlink_logs.txt')
    ...
    def __setConfigurationFiles(self, node:Node):
        ...
        node.setFile('/api.txt', ChainlinkFileTemplate['api'].format(username=self.__username, password=self.__password))
        node.setFile('/jobs/getUint256.toml', ChainlinkJobsTemplate['getUint256'])
        node.setFile('/jobs/getBool.toml', ChainlinkJobsTemplate['getBool'])
