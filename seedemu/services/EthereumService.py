...
class EthereumServer(Server):
    ...
    def install(self, node: Node, eth: 'EthereumService', allBootnode: bool):
        ...
        common_args = '{} --identity="NODE_{}" --networkid=10 --verbosity=2 --mine --allow-insecure-unlock --http --http.addr 0.0.0.0'.format(datadir_option, self.__id)
        if self.isRemixNode():
            remix_args = "--http.port 5555 --http.corsdomain '*' --http.api web3,eth,debug,personal,net"
