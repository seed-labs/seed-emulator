...
class NamingoRegistrarServer(Server):
    ...
    def __init__(self):
        ...
        self.__db_port = 3306
        self.__external_database_wait_timeout = 300
    ...
    def setExternalDatabase(
        ...
        assert wait_timeout > 0, "database wait timeout must be positive"
        self.setDatabase(name, username, password)
        self.__db_host = host
        self.__db_port = port
        self.__external_database_wait_timeout = wait_timeout
    ...
    def setIdentity(self, identity: RegistrarIdentity) -> NamingoRegistrarServer:
        ...
        self.__rdap_url = identity.rdap_url.rstrip("/")
        self.__abuse_email = identity.abuse_email
        self.__abuse_phone = identity.abuse_phone
        return self
...
if [ -s /opt/seedemu/namingo/schema.sql ] && [ ! -e /var/lib/mysql/.seedemu-namingo-schema ]; then
    ...
    touch /var/lib/mysql/.seedemu-namingo-schema
            ...
            database_setup = (
                ...
                "{database} -e 'SELECT 1' >/dev/null 2>&1; do\n"
                "    attempts=$((attempts + 1))\n"
                "    if [ \"$attempts\" -ge {wait_timeout} ]; then\n"
                "        echo 'Namingo Registrar could not reach its external database "
                "within {wait_timeout} seconds' >&2\n"
                "        exit 1\n"
                "    fi\n"
                "    sleep 1\n"
                "done"
...
""".format(
    ...
    def install(self, node: Node):
        ...
        if self.__expose_rdds_to_agent:
            node.setLabel(AGENT_RDDS_LABEL_META.format(key="authority"), "registrar")
            if self.__enable_whois:
                node.setLabel(
                    AGENT_RDDS_LABEL_META.format(key="whois_server"),
                    self.__registrar_whois,
                )
            if self.__enable_rdap:
                node.setLabel(
