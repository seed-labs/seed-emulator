...
class Customizable(object):
    ...
    def __init__(
        self,
    ):  # scope param. actually only for debug/tests  , scope: Scope = None
        super().__init__()
        self._config = {}
        self._scope = None
    ...
    def scope(self) -> Scope:
        ...
        if not self._scope:
            return Scope(
                ScopeTier.Global
            )  # maybe introduce a ScopeTier.NONE for this...
        else:
            return self._scope

    def getScopedOption(
        self, key: str, scope: Scope = None
    ) -> Optional[Tuple[BaseOption, Scope]]:
        ...
        possible_scopes = [
            (
                Scope(
                    ...
                    scope._node_type,
                    as_id=scope.asn,
                    node_id=scope._node_id,
                )
                if scope._node_id and scope._as_id and scope._node_type
                else None
            ),  # Node-specific + type
            (
                Scope(ScopeTier.Node, node_id=scope._node_id, as_id=scope._as_id)
                if scope._node_id and scope._as_id
                else None
            ),  # Node-specific
            (
                Scope(ScopeTier.AS, scope._node_type, as_id=scope._as_id)
                if scope._as_id and scope._node_type
                else None
            ),  # AS & Type
            (
                Scope(ScopeTier.AS, ScopeType.ANY, as_id=scope._as_id)
                if scope._as_id
                else None
            ),  # AS-wide
    ...
    def handDown(self, child: "Customizable"):
        ...
        try:  # scopes could be incomparable
            ...
            ), "logic error - cannot inherit options from more general scopes"
        except:
            pass
    ...
    def setOption(self, op: BaseOption, scope: Scope = None):
        ...
        if (
            not op.name in self._config
        ):  # fst encounter of this option -> no conflict EASY
    ...
    def getScopedRuntimeOptions(
        self, scope: Scope = None
    ) -> List[Tuple[BaseOption, Scope]]:
