...
def makeVictim(emu: Emulator, base: Base, web: WebService, victim_asn: int):
    ...
    # web setting
    victim_server.appendStartCommand('tc qdisc replace dev net0 root tbf rate 10mbit burst 32kbit latency 400ms',
                                     fork=True)
    victim_server.addPortForwarding(8888, 80)
    print(f"Victim server created in AS {victim_asn} and serving a website.")


def run(dumpfile=None, hosts_per_as=8):
    ###############################################################################
    ...
    for stub_as in [150, 151, 152, 153, 154, 160, 161, 162, 163, 164, 170, 171]:
        ...
        for hostname in hosts:
            ...
            host.appendStartCommand(
                'echo -e "telnet stream tcp nowait root /usr/sbin/tcpd /usr/sbin/in.telnetd -L /bin/login" > /etc/inetd.conf',
                fork=True)
    ...
    # set C2 server to 10.170.0.100
    # use BotnetService to setup C2
    C2 = base.getAutonomousSystem(170).createHost('c2_server').joinNetwork('net0', address='10.170.0.100')
    ...
    emu.addBinding(Binding('bot-controller', filter=Filter(ip='10.170.0.100'), action=Action.FIRST))
    ...
    C2.appendStartCommand('python3 -m http.server 80 --directory /var/www/html',
                          fork=True)  # start C2 server(for mirai.py download purpose)
    ...
    if dumpfile is not None:
        ...
        emu.render()
        # change the docker base to mirai-base before compile
