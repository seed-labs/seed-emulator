...
DOCKER_COMPOSE_ETHER_VIEW = """
    seedemu-ether-client:
        image: handsonsecurity/seedemu-multiarch-etherview:buildx-latest
...
# Enable internetMap
# Enable etherView
docker = Docker(internetMapEnabled=True, etherViewEnabled=False, platform=platform)
docker.attachCustomContainer(
    compose_entry=DOCKER_COMPOSE_ETHER_VIEW,
    asn=152, net='net0', ip_address='10.152.0.80',
    port_forwarding="5000:5000/tcp", env=[f"DEFAULT_ROUTE={emu.getDefaultRouterByAsnAndNetwork(152, 'net0')}"]
)
