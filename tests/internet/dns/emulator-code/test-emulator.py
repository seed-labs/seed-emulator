...
# Build a minimal Internet with authoritative DNS, a local caching DNS, and a client host.
emu     = Emulator()
base    = Base()
routing = Routing()
ebgp    = Ebgp()
...
# Use Provider relationship to ensure reachability
...
# Root zone
dns.install('a-root').addZone('.').setMaster()
dns.install('b-root').addZone('.')
# com. TLD and a business zone example32.com.
dns.install('ns-com').addZone('com.')
dns.install('ns-example32-com').addZone('example32.com.').setMaster()
# Reverse zone in-addr.arpa.
dns.install('ns-rev').addZone('in-addr.arpa.').setMaster()
...
# Forward a business zone to its authoritative master
ldns_server.addForwardZone('example32.com.', 'ns-example32-com')
...
# Bind authoritative vnodes to AS150, and LDNS to the local-dns host in AS151
