...
class DNSTestCaseNoMaster(SeedEmuTestCase):
    ...
    def test_forwarders_fallback_by_vnode_name(self):
        self.assertIsNotNone(self.local_dns)
        # No master was set; install() should fallback to resolving vnode name to IP
        self.assertTrue(self._ok(self.local_dns, "grep -q 'zone \"example32.com.\"' /etc/bind/named.conf.local"))
        # forwarders stanza should exist (search whole file for robustness)
    ...
    def get_test_suite(cls):
        ...
        test_suite.addTest(cls('test_forwarders_fallback_by_vnode_name'))
        test_suite.addTest(cls('test_resolution_works'))
        return test_suite

if __name__ == "__main__":
    test_suite = DNSTestCaseNoMaster.get_test_suite()
    res = ut.TextTestRunner(verbosity=2).run(test_suite)

    DNSTestCaseNoMaster.printLog("==========Test=========")
    num, errs, fails = res.testsRun, len(res.errors), len(res.failures)
    DNSTestCaseNoMaster.printLog("score: %d of %d (%d errors, %d failures)" % (num - (errs+fails), num, errs, fails))
