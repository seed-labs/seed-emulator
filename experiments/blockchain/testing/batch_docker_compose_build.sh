...
# Retrieve all service names from docker-compose.yml
services=($(docker compose config --services))
total=${#services[@]}
echo "Total number of services: $total" | tee -a "$LOG_FILE"

if [ $total -eq 0 ]; then
    echo "? No services found in docker-compose.yml. Exiting script." | tee -a "$LOG_FILE"
    exit 1
fi
...
for ((i=0; i<total; i+=BATCH_SIZE)); do
    ...
    # Execute build
    docker compose build "${batch_services[@]}"
    status=$?
